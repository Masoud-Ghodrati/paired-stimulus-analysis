...
select_Electrodes = [1:14 16:19 21 22 26 27 29 31 32 37 40:42 44 46 47 50:58 61:70 73 75 76 81 83:88 91 93:96 ];
...
stim_Sample    = unique(stim.allStimTrain(3,:));
    ...
    for iTrailStim = 1 : length(stim_TrailStim)
        ...
        for iLeadStim = 1 : length(stim_LeadStim)
            ...
            if iTrailStim == iLeadStim
                subplot(1,6,subplot_Ind)
                cInd = 1;
                for iSample = 1 : length(stim_Sample)
                    
                    this_Unique_Pair   = stim_Train(1, :) == stim_LeadStim(iLeadStim) &  stim_Train(2, :) == stim_TrailStim(iTrailStim);
                    this_Unique_Sample = stim_Train(3, :) == stim_Sample(iSample);
                    this_Pair          = find(this_Unique_Pair & this_Unique_Sample);
                    resps              = [];
                    ...
                    all_Resp{subplot_Ind}{iSample}(:,:, iElectrode) = resps;
                    plot(mean(resps), 'color', c(cInd, :)), hold on
                    cInd = cInd + 6;
                end
...
for iPair = 1 : 6
    subplot(1,6,iPair)
    for iSample =  1 : length(stim_Sample)
        plot(mean(mean(all_Resp{iPair}{iSample}), 3))
        hold on
    end
