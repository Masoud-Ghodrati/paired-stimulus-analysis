...
for iElectrode = 1 : length(select_Electrodes)
    ...
    mx = mean(all_Resp);
    [mymx, myind] = max(mx(150:end));
     plot(myind+150, mymx, 'ro'), hold on
    if FigureTab
    ...
    cell_Ind_Max(iElectrode) = myind+150; 
end
    ...
    for iTrailStim = 1 : length(stim_TrailStim)
        ...
        for iLeadStim = 1 : length(stim_LeadStim)
            ...
            if iTrailStim == iLeadStim
                %                 predicted_Resp{1, iTrailStim}(iElectrode, :) =  mean(resps - mean(resps(:, baseline_Window), 2));
                predicted_Resp{1, iTrailStim}(iElectrode) =  mean(mean(resps(360:end, cell_Ind_Max(iElectrode)-30:cell_Ind_Max(iElectrode)+30), 2));
            else
        ...
        %         nonpredicted_resp{1, iTrailStim}(iElectrode, :) = mean(temp_Resp - mean(temp_Resp(:, baseline_Window), 2));
        nonpredicted_resp{1, iTrailStim}(iElectrode, :) = mean(mean(temp_Resp(:, cell_Ind_Max(iElectrode)-30:cell_Ind_Max(iElectrode)+30), 2));
        
%         plot(predicted_Resp{1, iTrailStim}(iElectrode, :), 'r'), hold on
%         plot(nonpredicted_resp{1, iTrailStim}(iElectrode, :), 'b')
...
for iTrailStim = 1 : length(stim_TrailStim)
    ...
    scatter(predicted_Resp{1, iTrailStim}, nonpredicted_resp{1, iTrailStim}, 'o', 'filled'), hold on
    plot(0:140, 0:140, ':k')
end
